...
GSRD.mapID_to_engMapName = {}
for n, i in pairs(GSRD.engMapName_to_mapID) do
...
function class:UpdateState(unit)
	if self.order < 10000 then
		...
		if
			true ~= self.states[unit] or self.count ~= self.counts[unit] or self.type ~= self.types[unit] or
