...
local function Bar_Update(self, parent, unit, status)
	if unit then
		...
		if status then
			...
			for i, st in ipairs(self.statuses) do
				values[i] = st:GetPercent(unit) or 0
			end
...
local function Bar_Layout(self, parent)
	...
	for i = 1, barCount do
		...
		if c then
			...
			ctextures = ctextures or {}
			ctextures[#ctextures + 1] = texture
		end
...
local function BarColor_SetBarColor(self, parent, r, g, b, a)
	...
	bar:SetStatusBarColor(r, g, b, min(self.opacity or 0, a or 1))
